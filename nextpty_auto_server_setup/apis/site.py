    ...
    for session_user_site in session_user_sites_details:
        ...
        for site in site_details:
            ...
            site_info.append({
                "site_name": site_name,
                "status": status,
                "subscription": get_subscription_status(subscription_name) if subscription == "Active" else subscription,
                "is_trial": is_trial,
                "save_card": get_save_card(session_user, site_name),
...
def get_save_card(session_user, site_name):
    return frappe.db.get_value("Croem Saved Card Token", filters={"user": session_user, "site_name": site_name}, fieldname=['card_number'])
